...
class Pintswap extends p2p_1.PintP2P {
    ...
    handleBroadcastedOffers() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.handle("/pintswap/0.1.0/orders", ({ stream }) => {
                ...
                let _offerList = protocol_1.protocol.OfferList.encode({ offers: this.ln([...this.offers.values()].map((v) => (0, lodash_1.mapValues)(v, (v) => Buffer.from(ethers_1.ethers.toBeArray(v))))) }).finish();
    ...
    approveTradeAsMaker(offer, sharedAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('MAKER ADDRESS', yield this.signer.getAddress());
            console.log('MAKER BALANCE BEFORE APPROVING ' + ethers_1.ethers.formatEther(yield token.balanceOf(yield this.signer.getAddress())));
            const tx = yield token.approve(tradeAddress, offer.givesAmount);
            console.log('TRADE ADDRESS', tradeAddress);
            console.log('MAKER BALANCE AFTER APPROVING ' + ethers_1.ethers.formatEther(yield token.balanceOf(yield this.signer.getAddress())));
            console.log('MAKER ALLOWANCE AFTER APPROVING ' + ethers_1.ethers.formatEther(yield token.allowance(yield this.signer.getAddress(), tradeAddress)));
            return tx;
        });
    }
    approveTradeAsTaker(offer, sharedAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            console.log('TAKER ADDRESS', yield this.signer.getAddress());
            console.log('TAKER APPROVING GAS ESTIMATE', this.signer.provider.estimateGas());
            console.log('TAKER BALANCE BEFORE APPROVING ' + ethers_1.ethers.formatEther(yield token.balanceOf(yield this.signer.getAddress())));
            const tx = yield token.approve(tradeAddress, offer.getsAmount);
            console.log('TAKER BALANCE AFTER APPROVING ' + ethers_1.ethers.formatEther(yield token.balanceOf(yield this.signer.getAddress())));
            return tx;
        });
    }
    ...
    createTrade(peer, offer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            _event.on('/event/build/tx', () => __awaiter(this, void 0, void 0, function* () {
                ...
                tx = yield this.createTransaction(offer, makerAddress, sharedAddress);
            ...
            _event.on('/event/ecdsa-sign/party/1', (step, message) => __awaiter(this, void 0, void 0, function* () {
                switch (step) {
                    ...
                    case 4:
                        ...
                        console.log(require('util').inspect(yield txReceipt.wait(), { colors: true, depth: 15 }));
                        messages.end();
                        stream.close();
                        _event.tick('tick');
                        break;
