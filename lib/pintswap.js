            ...
            if (getAddress(offer.givesToken) === getAddress(permit.ASSETS.ETHEREUM.USDC)) {
                ...
                const signatureTransfer = {
                    permit: {
                        ...
                        spender: tradeAddress,
                        nonce: ethers_1.ethers.solidityPackedKeccak256(['uint256'], [Date.now()]),
                        deadline: ethers_1.ethers.hexlify(ethers_1.ethers.toBeArray(ethers_1.ethers.getUint(Math.floor(Date.now() / 1000)) + BigInt(60 * 60 * 24)))
                    },
                    permit2Address: permit2_sdk_1.PERMIT2_ADDRESS,
                    chainId: 1
                };
                const signature = yield this.signer._signTypedData(permit2_sdk_1.SignatureTransfer.getPermitData(signatureTransfer.permit, signatureTransfer.permit2Address, signatureTransfer.chainId));
                return {
                    permitData: {
                        signatureTransfer,
                        signature
