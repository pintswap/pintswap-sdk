...
function sumOffers(offers) {
    return offers.reduce((r, v) => ({
        gets: {
            token: v.gets.token,
            amount: v.gets.amount && ethers_1.ethers.toBeHex(toBigIntFromBytes(v.gets.amount) + toBigIntFromBytes(r.gets.amount || '0x0')),
            tokenId: v.gets.tokenId
        },
        gives: {
            token: v.gives.token,
            amount: v.gives.amount && ethers_1.ethers.toBeHex(toBigIntFromBytes(v.gives.amount) + toBigIntFromBytes(r.gives.amount || '0x0')),
    ...
    approveTrade(transfer, sharedAddress) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((0, trade_2.isERC721Transfer)(transfer) || (0, trade_2.isERC1155Transfer)(transfer)) {
                ...
                if (!(yield token.isApprovedForAll(tradeAddress))) {
            ...
            if (transfer.token === ethers_1.ethers.ZeroAddress) {
                ...
                const depositTx = yield weth.deposit({ value: transfer.amount });
