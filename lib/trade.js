...
const permit2Interface = new ethers_1.ethers.Interface(Permit2_json_1.default);
// UTILS
function toBigInt(v) {
    if (v.toHexString)
        return v.toBigInt();
...
const createContract = (offer, maker, taker, chainId = 1, permitData = {}) => {
    ...
    const transferFrom = (token, from, to, amount, permitData) => {
        if (permitData && permitData.signatureTransfer) {
            if (token === ethers_1.ethers.ZeroAddress) {
                return [
                    call(permit2_sdk_1.PERMIT2_ADDRESS, permit2Interface.encodeFunctionData("permitTransferFrom", [
                        {
                            permitted: {
            ...
            return call(permit2_sdk_1.PERMIT2_ADDRESS, permit2Interface.encodeFunctionData("permitTransferFrom", [
                {
    ...
    return (0, emasm_1.emasm)([
        ...
        transferFrom(offer.givesToken, maker, taker, offer.givesAmount, permitData.m && permitData.maker),
        "iszero",
        "failure",
        "jumpi",
        getAddress(maker),
